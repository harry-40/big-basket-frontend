...
import { Avatar,InputAdornment,TextField,Button,Grid,IconButton,FormControl,
  InputLabel,
  OutlinedInput,
  Select,
  MenuItem,
    ...
    <div className={classes.mainContainer}>
      <div className={classes.box}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
          <TextField error={!error.companyId?false:true} helperText={error.companyId} onFocus={()=>handleError("companyId",null)}  onChange={(event) => setCompanyId(event.target.value)} value={companyId} fullWidth label="Company Id" variant="outlined" />
          </Grid>
          <Grid item xs={6}>
