...
export default function Cart() {
          ...
          <Grid item xs={12} md={5}>
            ...
            <Grid item xs={11.6} p={2} py={4} mx="auto" bgcolor="white" mt={3} container  gap={3}>
              <div style={{ display: "flex",flexDirection:'column' }}>
               <div style={{ display: "flex",alignItems:'center',justifyContent:'center',width:500 }} >
                <LocationOnIcon fontSize="large" color="primary" />
                <b style={{ color: "gray" }}>Delivery address</b>
                </div>
                <div style={{display:'flex',flexDirection:'column',fontFamily:'Poppins',fontSize:'large',color: 'grey'}}>
  ...
  <div>{userdata[0]?.zipcode}</div></>:<></>}
      ...
      <LoginDialog setBtnMsg={setBtnMsg} userData={userData} setUserData={setUserData} setDialogState={setDialogState} state={dialogState} pageRefresh={pageRefresh} setAddressState={setAddressState}/>
