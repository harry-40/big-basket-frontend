...
export default function ExplorCategory(){
  ...
  { return categoryList.map((item)=>{
        ...
        '10%',margin:3,display:'flex',alignItems:'center',flexDirection:'column'}}><div style={{color:'#792c85',fontFamily:'Poppins',
            fontSize:xs?4:sm?6:md?8:lg?16:16,fontWeight:'bolder',textAlign:'center'}}>{item.categoryname}</div><img src={`${ServerURL}/images/${item.icon}`} style={{width:'60%'}}/></div>)
  })
  } 
